...
/* ====================================================================
 * openwatcomirc: ELF64 output path for x86_64 and arm64 targets
 * Parallel to the Elf32 code above. Cleanly separated.
 * ==================================================================== */

static bool isElf64Cpu( owl_cpu cpu ) {
    return( cpu == OWL_CPU_X64 || cpu == OWL_CPU_A64 );
}

static void writeFileHeader64( owl_file_handle file )
{
    Elf64_Ehdr h;
    memset( &h, 0, sizeof(h) );
    ...
    h.e_type = ET_REL;
    h.e_machine = machineTypes[file->info->cpu];
    h.e_version = EV_CURRENT;
    h.e_shoff.u._64[0] = sizeof(h);
    h.e_ehsize = sizeof(h);
    h.e_phentsize = sizeof(Elf64_Phdr);
    h.e_shentsize = sizeof(Elf64_Shdr);
    h.e_shnum = numSections( file );
    h.e_shstrndx = ELF_STRING_INDEX;
    _ClientWrite( file, (const char *)&h, sizeof(h) );
}

static void emitElfSymbol64( owl_symbol_info *symbol, Elf64_Sym *s )
{
    s->st_name = OWLStringOffset( symbol->name );
    s->st_info = ELF64_ST_INFO( elfBinding[symbol->linkage], elfType[symbol->type] );
    s->st_other = 0;
    s->st_shndx = SHN_UNDEF;
    if( symbol->section != NULL )
        s->st_shndx = _OWLIndexToELFIndex( symbol->section->index );
    s->st_value.u._64[0] = symbol->offset;
    s->st_size.u._64[0] = 0;
}

static void prepareSymbolTable64( owl_file_handle file, elf_special_section *sym_sect )
{
    owl_symbol_info *sym;
    unsigned next_local_index, next_global_index;
    Elf64_Sym *syms;

    sym_sect->length = ( numSymbols(file) + 1 ) * sizeof(Elf64_Sym);
    sym_sect->buffer = _ClientAlloc( file, sym_sect->length );
    syms = (Elf64_Sym *)sym_sect->buffer;
    memset( &syms[0], 0, sizeof(Elf64_Sym) ); /* bogus symbol */
    next_local_index = 1;
    next_global_index = file->symbol_table->num_global_symbols + file->symbol_table->num_local_symbols;
    for( sym = file->symbol_table->head; sym != NULL; sym = sym->next ) {
        ...
        emitElfSymbol64( sym, &syms[sym->index] );
    }
    assert( (next_global_index + 1) == next_local_index );
}

static void initShdr64( Elf64_Shdr *h, uint_32 type, uint_32 flags )
{
    memset( h, 0, sizeof(*h) );
    h->sh_type = type;
    h->sh_flags.u._64[0] = flags;
}

static void emitSectionHeaders64( owl_file_handle file )
{
    unsigned n = numSections( file );
    uint_32 table_size = n * sizeof(Elf64_Shdr);
    Elf64_Shdr *hdrs = _ClientAlloc( file, table_size );
    owl_section_handle curr;
    elf_special_section *sym_tab = &file->x.elf.symbol_table;
    size_t reloc_entry_size = useRela ? sizeof(Elf64_Rela) : sizeof(Elf64_Rel);

    file->x.elf.next_section = sizeof(Elf64_Ehdr) + table_size;

    /* [0] null */
    initShdr64( &hdrs[ELF_UNDEF_INDEX], SHT_NULL, 0 );

    /* [1] .strtab */
    initShdr64( &hdrs[ELF_STRING_INDEX], SHT_STRTAB, 0 );
    hdrs[ELF_STRING_INDEX].sh_name = OWLStringOffset( file->x.elf.string_table.name );
    hdrs[ELF_STRING_INDEX].sh_offset.u._64[0] = file->x.elf.next_section;
    hdrs[ELF_STRING_INDEX].sh_size.u._64[0] = file->x.elf.string_table.length;
    file->x.elf.next_section += file->x.elf.string_table.length;

    /* [2] .symtab */
    initShdr64( &hdrs[ELF_SYMBOL_INDEX], SHT_SYMTAB, 0 );
    hdrs[ELF_SYMBOL_INDEX].sh_name = OWLStringOffset( sym_tab->name );
    hdrs[ELF_SYMBOL_INDEX].sh_link = ELF_STRING_INDEX;
    hdrs[ELF_SYMBOL_INDEX].sh_info = file->symbol_table->num_local_symbols + 1;
    hdrs[ELF_SYMBOL_INDEX].sh_entsize.u._64[0] = sizeof(Elf64_Sym);
    hdrs[ELF_SYMBOL_INDEX].sh_offset.u._64[0] = file->x.elf.next_section;
    hdrs[ELF_SYMBOL_INDEX].sh_size.u._64[0] = sym_tab->length;
    hdrs[ELF_SYMBOL_INDEX].sh_addralign.u._64[0] = 8;
    file->x.elf.next_section += sym_tab->length;

    /* user sections */
    for( curr = file->sections; curr != NULL; curr = curr->next ) {
        Elf64_Shdr *h = &hdrs[_OWLIndexToELFIndex( curr->index )];
        initShdr64( h, sectionTypes(curr->type), sectionFlags(curr->type) );
        h->sh_name = OWLStringOffset( curr->name );
        h->sh_addralign.u._64[0] = sectionAlignment( curr );
        h->sh_size.u._64[0] = curr->size + sectionPadding( curr, curr->size );
        if( (curr->type & OWL_SEC_ATTR_BSS) == 0 ) {
            h->sh_offset.u._64[0] = file->x.elf.next_section;
            file->x.elf.next_section += h->sh_size.u._64[0];
        }
        if( curr->first_reloc != NULL ) {
            Elf64_Shdr *rh = &hdrs[_OWLIndexToELFIndex( curr->x.elf.relocs_index )];
            initShdr64( rh, (useRela ? SHT_RELA : SHT_REL), sectionFlags(curr->type) & SHF_ALLOC );
            rh->sh_name = OWLStringOffset( curr->x.elf.relocs_name );
            rh->sh_link = ELF_SYMBOL_INDEX;
            rh->sh_info = _OWLIndexToELFIndex( curr->index );
            rh->sh_size.u._64[0] = reloc_entry_size * curr->num_relocs;
            rh->sh_entsize.u._64[0] = reloc_entry_size;
            rh->sh_addralign.u._64[0] = 8;
            rh->sh_offset.u._64[0] = file->x.elf.next_section;
            file->x.elf.next_section += rh->sh_size.u._64[0];
        }
    }

    _ClientWrite( file, (const char *)hdrs, table_size );
    _ClientFree( file, hdrs );
}

static void emitReloc64( owl_section_handle sec, owl_reloc_info *reloc, Elf64_Rela *r )
{
    owl_offset old_loc;
    unsigned_32 data;
    unsigned_32 bit_mask;
    ...
    r->r_offset.u._64[0] = reloc->location;
    r->r_info.u._64[0] = ELF64_R_INFO( (uint_64)reloc->symbol->index,
                                         ElfRelocType( reloc->type, sec->file->info->cpu ) );
    bit_mask = OWLRelocBitMask( sec->file, reloc );
    if( useRela ) {
        old_loc = OWLBufferTell( sec->buffer );
        OWLBufferSeek( sec->buffer, reloc->location );
        ...
        OWLBufferRead( sec->buffer, reloc->location, (char *)&data, data_size );
        r->r_addend.u._64[0] = (int_64)(int_32)( data & bit_mask );
        data &= ~bit_mask;
        OWLBufferWrite( sec->buffer, (char *)&data, data_size );
        OWLBufferSeek( sec->buffer, old_loc );
    }
...
static void emitSectionData64( owl_file_handle file )
{
    owl_section_handle curr;
    owl_reloc_info *reloc;
    owl_offset relocs_size;
    ...
    for( curr = file->sections; curr != NULL; curr = curr->next ) {
        ...
        if( curr->first_reloc != NULL ) {
            ...
            next = reloc_buffer;
            for( reloc = curr->first_reloc; reloc != NULL; reloc = reloc->next ) {
                emitReloc64( curr, reloc, next );
                next = (Elf64_Rela *)( (char *)next + reloc_entry_size );
            }
        }
        if( !_OwlSectionBSS( curr ) ) {
            OWLBufferEmit( curr->buffer );
        ...
        if( reloc_buffer != NULL ) {
            _ClientWrite( file, (const char *)reloc_buffer, reloc_entry_size * curr->num_relocs );
            _ClientFree( file, reloc_buffer );
        }
    }
}

void ELFFileEmit64( owl_file_handle file )
{
    prepareRelocSections( file );
    writeFileHeader64( file );
    addSpecialStrings( file );
    prepareStringTable( file, &file->x.elf.string_table );
    prepareSymbolTable64( file, &file->x.elf.symbol_table );
    emitSectionHeaders64( file );
    emitSpecialSection( file, &file->x.elf.string_table );
    emitSpecialSection( file, &file->x.elf.symbol_table );
    emitSectionData64( file );
}
