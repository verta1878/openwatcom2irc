...
/* openwatcomirc: x64 ELF64 dispatch (from x64dispatch.c) */
...
/* openwatcomirc: dummy segment record for x64 mode.
 * When X64IsActive(), CurrSeg points here instead of NULL.
 * Prevents crashes in functions that dereference CurrSeg. */
static index_rec    x64_dummy_seg;
static object       x64_dummy_obj;
...
/*********************/
{
    ...
    /* openwatcomirc: check for x86_64 ELF64 target */
    /* When -bt=linux64 sets CGSW_GEN_OBJ_ELF, redirect through OWL */
    /* x64dispatch.obj is linked — X64CheckDispatch() is available */
#if 1 /* x64dispatch.obj linked into build as of Phase 20+ */
    if( X64CheckDispatch() ) {
        /* x64 ELF64 output via OWL — Phase 21 */
...
/*********************/
{
    ...
    /* openwatcomirc: x64 ELF64 — post-processing happens after CloseObj below */
#if 0 /* moved to after CloseObj */
    if( X64IsActive() ) {
        /* x64 ELF64 finalization via OWL */
        X64ObjFini();
        /* Fall through to OMF cleanup */
    }
#endif
    ...
    EndModule();
    CloseObj();
    /* openwatcomirc: post-process OMF → ELF64 after file is closed */
    if( X64IsActive() ) {
        X64ObjFini();
    }
