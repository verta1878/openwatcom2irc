...
/* openwatcomirc: x86_64 REX prefix accumulator.
 * REX byte format: 0100WRXB
 *   W=1: 64-bit operand size
 *   R=1: extends ModRM.reg to R8-R15
 *   X=1: extends SIB.index to R8-R15
 *   B=1: extends ModRM.rm or SIB.base to R8-R15
 * Accumulated during instruction encoding, inserted before opcode
 * by TransferIns(). Only active when x64_mode is true.
 */
static  byte        RexAccum;       /* accumulated REX bits */
static  bool        RexNeeded;      /* true if any REX bit was set */
static  bool        x64_mode;       /* true when targeting x86_64 */

static void ResetRex( void ) { RexAccum = 0; RexNeeded = false; }
static void SetRexW( void ) { if( x64_mode ) { RexAccum |= 0x08; RexNeeded = true; } }
static void SetRexR( void ) { if( x64_mode ) { RexAccum |= 0x04; RexNeeded = true; } }
static void SetRexX( void ) { if( x64_mode ) { RexAccum |= 0x02; RexNeeded = true; } }
static void SetRexB( void ) { if( x64_mode ) { RexAccum |= 0x01; RexNeeded = true; } }

void X64EncInit( bool is_x64 ) { x64_mode = is_x64; ResetRex(); }
#if 0
 ...
 * openwatcomirc: inserts REX prefix for x86_64 when accumulated
 */
{
    unsigned i;
    unsigned j;

    /* openwatcomirc: Patch 6 — remap single-byte INC/DEC for x86_64.
     * In 64-bit mode, opcodes 0x40-0x4F are REX prefixes.
    ...
    /* openwatcomirc: insert REX prefix if accumulated during encoding.
     * REX goes after legacy prefixes (66h, F2h, F3h) but before the opcode.
     * We find the opcode position, shift right, and insert REX. */
    if( RexNeeded && x64_mode ) {
        unsigned opcode_pos = 0;
        /* Skip legacy prefixes */
        while( opcode_pos < ICur ) {
 ...
 * openwatcomirc: sets REX.R if register is R8-R15
 */
{
    /* openwatcomirc: detect extended registers for REX.R */
    if( x64_mode && (r->r.reg._1 & 0x000003FCU) )
        SetRexR();
 ...
 * openwatcomirc: sets REX.B if register is R8-R15
 */
{
    if( x64_mode && (r->r.reg._1 & 0x000003FCU) )
        SetRexB();
